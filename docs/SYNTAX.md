...
When a variant or conditional produces no output, adjacent commas in the surrounding text are automatically merged into one:

```
thing, {0$$a|b}, other_thing   →  thing, other_thing
a, {0$$x|y}, {0$$x|y}, b      →  a, b
```

This is enabled by default. When using the library directly, it can be disabled by passing `squash_commas=False` to the generator constructor:
